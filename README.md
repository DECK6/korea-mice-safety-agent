...
`/live`는 행사 당일용 현장 라이브 대시보드이고, 탭 3개로 나뉩니다.

- **홈 · 행사 지도**(기본): 지도가 화면 대부분을 차지하는 상황판입니다. 로컬 운영 저장소의 이슈·조치·런시트·지휘판단을 행사 단위로 집계해 상태색 마커로 올리고, 옆 레일에 행사 목록, 선택 행사의 작업 리스트(이슈·조치 혼합, SLA 급한 순 상위 8건, 팀·기한 표시), 위치 미지정 행사 목록을 붙입니다. 상태는 `critical`(활성 대피·중단 지휘판단 또는 critical 이슈 SLA 초과), `watch`(활성 지휘판단·런시트 막힘·SLA 초과/임박·high 이슈), `normal`로 판정합니다. 우상단 인파·폭염 배지를 누르면 실시간 신호 탭으로 넘어갑니다. 등록된 행사가 없으면 데모 없이 빈 상태와 등록 방법만 안내합니다.
- **실시간 신호**: 조회 조건, 인파(서울 실시간 도시데이터 핫스팟 + 전국 SKT 수동 모드), 날씨(체감온도 33/35°C 폭염 배지), 대기질, 교통·재난문자(키 발급 전 pending 표시), AI 상황 브리핑 카드가 여기 있습니다. 60초 간격으로 갱신하며 `GET /api/live-status`로 같은 데이터를 JSON으로 받을 수 있습니다.
- **법령·의무 문서**: 오프라인 온톨로지 적용성 조회. 버튼을 누를 때만 실행합니다.

행사 위치는 `GET /api/operations-map`이 집계해 내려주고, `POST /api/operations-map/location`(`{eventName, venueId}` 또는 `{eventName, lat, lng, label}`)으로 지정합니다. 지정 결과는 `MICE_LOCAL_DIR/event-locations.json`에 임시파일+rename으로 저장하며, 이 파일은 표시 위치일 뿐이라 `operations.json`의 해시체인 밖에 둡니다. 표시 좌표가 등록된 베뉴만 셀렉트에 나오고, 그 밖의 장소는 좌표를 직접 넣어야 합니다. 홈 탭도 로컬 파일만 읽으므로 60초 자동 갱신 대상이고, SKT 실호출과 AI 브리핑만 수동으로 남습니다.

화면은 DEXA 디자인 시스템의 **다크 전시 표면(exhibition surface)** 문법을 따릅니다. 색·타이포 토큰은 정본 `dexa-theme.css`의 이름과 값을 그대로 씁니다(`--ink #17181B`, `--ink-display #0D0E10`, `--ink-key #2A2B2E`, `--ink-key2 #3A3B3F`, `--ink-text #8A8D93`, `--ink-text-dim #5A5D63`, 헤딩 `#F7FAFC`, `--cyan #5EE7F3`, `--red #E0402A`, `--font-sans` Space Grotesk 계열 / `--font-mono` JetBrains Mono 계열). 상태색 `--ok`(그린)·`--watch`(앰버)만 정본에 없는 시맨틱 확장인데, 운영 상태를 빨강 하나로 표현할 수 없어 `:root`에 따로 정의했습니다. 주요 패널(상태 스트립, 지도 카드)은 정본 `.hero-panel` 문법대로 16px 라운드 + 모서리 스크류 도트 + mono 상태줄(좌측 경로 라벨, 우측 시안 LIVE 점멸, `prefers-reduced-motion` 대응)을 쓰고, 수치는 `.hero-display` 문법의 함몰 디스플레이(mono·시안)에 올립니다. 라이트 크롬 전용인 시그널 오렌지는 이 다크 대시보드에 쓰지 않습니다. 웹폰트는 로드하지 않고 시스템 폴백만 씁니다.

Leaflet은 CDN이 아니라 빌드 시 `build/web/vendor/leaflet/`으로 복사해 `/vendor/`에서 자체 서빙하며(OpenStreetMap 표준 타일 + attribution, 타일 서버에 닿지 못하면 `지도 타일 오프라인` 안내로 대체), 지도 인스턴스는 홈 탭이 소유하고 탭 전환 시 `invalidateSize()`로 다시 측정합니다. 타일은 채도를 걷어내 `--ink-display` 평면 위 중성 다크 베이스맵으로 톤을 맞춥니다. 실시간 값도 표시 좌표도 법적 근거가 아니라 운영 참고용이고, 좌표는 지점 식별용 근사값이라 거리·수용인원 계산에는 쓰지 않습니다.

인파 카드는 `전국 SKT(수동)` 모드로 전환하면 서울 밖 장소도 볼 수 있습니다. 장소 검색은 번들된 오프라인 POI 인덱스(상위 1,000곳, `GET /api/skt-pois?q=`)라 무료이고, SKT 지오비전 퍼즐 장소 혼잡도 실호출(`GET /api/skt-congestion?poiId=`)은 무료 플랜 월 10건 한도 때문에 자동 갱신에서 제외하고 `혼잡도 조회` 버튼을 누를 때만 1건을 사용합니다. 사용량은 로컬 원장(`~/.korea-mice-safety-agent/skt-quota.json`)의 추정치로 표시하며, 한도에 닿으면 호출하지 않고 다음 달 리셋을 안내합니다.
