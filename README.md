...
관제실형 다크 화면에는 지도 패널이 함께 붙습니다. Leaflet은 CDN이 아니라 빌드 시 `build/web/vendor/leaflet/`으로 복사해 `/vendor/`에서 자체 서빙하고(OpenStreetMap 표준 타일 + attribution, 타일 서버에 닿지 못하면 `지도 타일 오프라인` 안내로 대체), 선택한 서울 인파 지역이나 전국 SKT 베뉴를 상태색 마커·원형으로 강조합니다. 표시 좌표는 지점 식별용 근사값이라 거리·수용인원 계산에는 쓰지 않습니다.
