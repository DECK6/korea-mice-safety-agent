...
export const API_ACCESS_DEFINITIONS: ApiAccessDefinition[] = [
  ...
  },
  {
    ...
    notes: "서울 장소별 인파·도시 데이터 snapshot 및 P2 live crowd adapter에 사용한다. 공개된 실시간 기지국 기반 인파 API는 서울 한정이며, 서울 외 지역에 일반화하지 않는다.",
  },
  {
    id: "mois_crowd_management",
    label: "행안부 인파관리지원시스템(전국 기지국 인파)",
    phase: "P2",
    statusWhenUnset: "externally_available",
    requiredForHappyPath: false,
    offlineMode: "snapshot",
    notes: "공개 API 없음. 전국 중점관리지역 약 100곳을 통신 기지국 신호로 관측하지만 지자체·소방·경찰 상황실 전용이다. 주최자는 관할 지자체 재난상황실과 공동대응 협의로 행사기간 인파 정보 공유를 요청해 확보한다.",
  },
  {
    id: "telecom_bdp_floating_population",
    label: "통신사 빅데이터 유동인구(KT BDP 등)",
    phase: "P2",
    statusWhenUnset: "pending",
    requiredForHappyPath: false,
    offlineMode: "snapshot",
    notes: "유료 계약 기반이며 실시간이 아니라 집계 주기가 있는 통계성 유동인구다. 당일 혼잡도 판단이 아니라 사전 수요 예측 참고로만 사용한다.",
  },
  {
    id: "skt_puzzle_place_congestion",
    label: "SKT 지오비전 퍼즐 장소 혼잡도(전국 실시간)",
    phase: "P2",
    envVar: "SK_OPEN_API_APP_KEY",
    statusWhenUnset: "pending",
    requiredForHappyPath: false,
  ...
  },
  {
    ...
    label: "한국관광공사 관광지 혼잡도(집중률) 예측",
    phase: "P2",
    envVar: "TOUR_API_SERVICE_KEY",
    statusWhenUnset: "pending",
    requiredForHappyPath: false,
    offlineMode: "snapshot",
    notes: "전국 관광지 대상 일 단위 혼잡도 예측(실시간 아님). 기존 data.go.kr TourAPI 키 계열로 활용 신청 가능. 행사 전 사전 수요 예측 참고용이며 당일 판단에는 실시간 소스를 쓴다.",
  },
  {
